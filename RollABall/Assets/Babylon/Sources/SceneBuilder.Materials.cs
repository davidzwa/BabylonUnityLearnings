...
namespace Unity3D2Babylon
{
    partial class SceneBuilder
    {
        ...
        private bool CopyTexture(string texturePath, Texture2D texture2D, BabylonTexture babylonTexture, bool isLightmap = false, bool isTerrain = false, bool asJpeg = false)
        {
            ...
            if (srcTexturePath.IndexOf("unity_builtin_extra", StringComparison.OrdinalIgnoreCase) >= 0) {
                string extraName = texture2D.name + ".png";
                string extraFile = Path.Combine(OutputPath, extraName);
                Texture2D copy = texture2D.RenderToTexture();
                if (copy.alphaIsTransparency) {
                    babylonTexture.hasAlpha = true;
                } else {
                    babylonTexture.hasAlpha = false;
                }
                copy.WriteImagePNG(extraFile);
                babylonTexture.name = extraName;
                babylonScene.AddTextureExtra(extraName);
                return true;
            }
            string textureExt = Path.GetExtension(texturePath);
            if (String.IsNullOrEmpty(textureExt)) {
                UnityEngine.Debug.LogWarning("Null extension encountered for path: " + texturePath);
                return false;
            }
            string sourcefile = Path.GetFileName(texturePath);
            if (String.IsNullOrEmpty(sourcefile)) {
                UnityEngine.Debug.LogWarning("Null source file encountered for path: " + texturePath);
                return false;
            }
            // Convert required file extensions
            bool needToDelete = false;
            string convertList = ".exr, .psd, .tif";
            if (exportationOptions.TruevisionGraphics == false) convertList += ", .tga";
            ...
            bool okextension = (nativepng || nativejpeg || (nativetga && exportationOptions.TruevisionGraphics == true));
            ...
            if (processImage == true) {
                ...
                if (isLightmap || isTerrain || enforceimage || convertList.IndexOf(textureExt, StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    ...
                    BabylonImageFormat textureFormat = BabylonImageFormat.JPG;
                    bool makeJpeg = (unityexr == false && hasAlpha == false && (asJpeg || nativejpeg || (enforceimage && exportationOptions.ImageEncodingOptions == (int)BabylonImageFormat.JPG)));
                    if (makeJpeg == false) {
                    ...
                    try
                    {
                        if (unityexr == true) {
                            ...
                            Tools.ColorCorrection colorCorrection = Tools.ColorCorrection.LinearToGamma;
                            Color[] pixels = Tools.ReadFreeImage(new FileStream(exrFilename, FileMode.Open, FileAccess.Read), ref readResult, ref readWidth, ref readHeight, ref readBitsPerPixel, colorCorrection);
                            if (readResult == true && pixels != null && pixels.Length > 0) {
                                var tempTexture = new Texture2D(readWidth, readHeight, TextureFormat.RGBAFloat, false);
                                tempTexture.SetPixels(pixels);
                                tempTexture.Apply();
                                tempTexture.WriteImagePNG16(texturePath);
                            } else {
                ...
                else if (texture2D.alphaIsTransparency)
                {
                    babylonTexture.hasAlpha = true;
                }
                else
            ...
            var textureLabel = Path.GetFileName(texturePath);
            babylonTexture.name = textureLabel;
        ...
        private BabylonMaterial DumpStandardMaterial(Material material, bool receiveShadows, int lightmapIndex = -1, Vector4 lightmapScaleOffset = default(Vector4), int lightmapCoordIndex = -1, BabylonDefaultMaterial defaultMaterial = null)
        {
            ...
            if (!materialsDictionary.ContainsKey(materialName)) {
                ...
                if (bMat.ambientTexture == null && hasLightmap) {
                    ...
                    {
                        ExporterWindow.ReportProgress(1, "Dumping standard material lightmap: " + lightmap.name);
        ...
        private BabylonMaterial DumpPBRMaterial(Material material, bool receiveShadows, int lightmapIndex = -1, Vector4 lightmapScaleOffset = default(Vector4), int lightmapCoordIndex = -1, bool metallic = true)
        {
            ...
            var babylonPbrMaterial = new BabylonSystemMaterial {
                ...
                cameraContrast = 1.0f,
                cameraExposure = 1.0f,
                reflectivity = Color.white.ToFloat(), // 0.2f, 0.2f, 0.2f, 1.0f - Default Darker - ???
            ...
            float defaultEmissiveIntensity = (SceneController != null) ? SceneController.lightingOptions.defaultShaderOptions.emissiveIntensity : 1.0f;
            float defaultSpecularIntensity = (SceneController != null) ? SceneController.lightingOptions.defaultShaderOptions.specularIntensity : 1.0f;
            ...
            float defaultSpecularIntensityFactor = (SceneController != null) ? SceneController.lightingOptions.defaultShaderOptions.specularHighlighting : 10.0f;
            ...
            if (material.HasProperty("_HighlightIntensity")) {
                defaultSpecularIntensityFactor = material.GetFloat("_HighlightIntensity");
            }
            ...
            DumpGlossinessReflectivity(material, metallic, defaultMicroSurfaceScaling, defaultSpecularIntensityFactor, babylonPbrMaterial);
            ...
            if (hasLightmap) {
                ...
                var texturePath = AssetDatabase.GetAssetPath(lightmap);
                if (!String.IsNullOrEmpty(texturePath)) {
                    ExporterWindow.ReportProgress(1, "Dumping physical material lightmap: " + lightmap.name);
        ...
        private void DumpGlossinessReflectivity(Material material, bool metallic, float microSurfaceScale, float highlightIntensity, BabylonSystemMaterial babylonPbrMaterial)
        {
            ...
            else
            {
                ...
                bool specularHighlights = (material.HasProperty("_SpecularHighlights") && material.GetFloat("_SpecularHighlights") != 0.0f);
                if (specularHighlights == true) {
                    babylonPbrMaterial.specularIntensity *= highlightIntensity;
                }
            }
        }
        ...
        private BabylonTexture DumpTexture(Texture texture, Material material = null, string name = "", bool isLightmap = false)
        {
            ...
            if (texture2D)
            {
                CopyTexture(texturePath, texture2D, babylonTexture, isLightmap, false, false);
            }
            else
            {
