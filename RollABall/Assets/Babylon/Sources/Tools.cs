...
namespace Unity3D2Babylon
{
    public static class Tools
    {
        ...
        public static Color? ColorFromFloats(float[] value)
        {
            Color? result = null;
            if (value != null) {
                if (value.Length == 3) {
        ...
        public static void CheckLayers(string[] layerNames)
        {
            ...
            // save
            if (save) manager.ApplyModifiedProperties();
        }
        ...
        public static List<AnimationParameters> GetAnimationParameters(Animation legacy)
        {
            return null; // Legacy Animation Does Not Support Animation Parameters
        }
        ...
        public static Dictionary<string, object> ExportStateMachine(Animation legacy, UnityEditor.AnimationState animationState)
        {
            int hash = 0;
            bool auto = (legacy != null) ? legacy.playAutomatically : false;
        ...
        public static Dictionary<string, object> ExportStateMachine(Animator animator, UnityEditor.AnimationState animationState)
        {
            ...
            Dictionary<string, object> stateInfo = new Dictionary<string, object>();
            stateInfo.Add("hash", hash);
            stateInfo.Add("auto", false);
            stateInfo.Add("legacy", false);
            stateInfo.Add("active", active);
            stateInfo.Add("speed", speed);
            stateInfo.Add("layers", layers);
            stateInfo.Add("states", states);
        ...
        public static MachineState CreateMachineState(Animator animator, AnimatorControllerLayer layer, int index, AnimatorStateMachine machine, AnimatorState state)
        {
            ...
            result.time = 0.0f;
            result.type = (state.motion != null && state.motion is BlendTree) ? (int)UnityEditor.MotionType.Tree : (int)UnityEditor.MotionType.Clip;
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        public static Dictionary<string, object> ParseBlendingTrees(Animator animator, int index, Motion motion, ref AnimatorState state, ref Motion rootMotion)
        {
            ...
            if (motion != null) {
                ...
                if (motion is AnimationClip ) {
                    ...
                    childInfo.Add("type", (int)UnityEditor.MotionType.Clip);
                    ...
                    if (tree.children != null && tree.children.Length > 0) {
                        foreach (var child in tree.children) {
                            ...
                            childInfo.Add("type", (child.motion != null && child.motion is BlendTree) ? (int)UnityEditor.MotionType.Tree : (int)UnityEditor.MotionType.Clip);
        ...
        {
            bool jpeg = (splitterFormat == BabylonImageFormat.JPG);
            ...
            foreach (CubemapFace face in Enum.GetValues(typeof(CubemapFace))) {
                ...
                var faceTexture = new Texture2D(cubemap.width, cubemap.height, TextureFormat.RGBAFloat, false);
                ...
                if (splitterFormat == BabylonImageFormat.PNG) {
                    faceTexture.WriteImagePNG16(faceTexturePath);
                } else {
                    faceTexture.WriteImageJPG(faceTexturePath);
                }
            }
        }
        ...
        public static void ValidateProjectLayers()
        {
            ...
            // Check Default Tags
            // ..
            Tools.CheckTags(new string[]{ "Navigation" });
        }
        ...
        public static void ExtractZipFile(string zipFileName, string outputDirectory, Ionic.Zip.ExtractExistingFileAction fileExtractAction = Ionic.Zip.ExtractExistingFileAction.OverwriteSilently)
        {
            if (File.Exists(zipFileName))
            {
                Ionic.Zip.ZipFile zip = Ionic.Zip.ZipFile.Read(zipFileName);
                if (!Directory.Exists(outputDirectory)) Directory.CreateDirectory(outputDirectory);
                zip.ExtractAll(outputDirectory, fileExtractAction);
            }
        }        
        ...
        public static int CalculateCameraDistance(float distance, float scale, float percent)
        {
            float bias = 1.0f; // Ignore Lod Bias For Distances - (QualitySettings.lodBias > 0.0f) ? QualitySettings.lodBias : 1.0f;
            return Mathf.RoundToInt(((distance * scale) * percent) * bias);
        }

        public static float GetCameraViewportDistance(LODGroup lodGroup)
        {
            return (lodGroup.transform.TransformPoint(lodGroup.localReferencePoint) - Camera.main.transform.position).magnitude;
        }
        ...
        {
            int result = 0;
            if (physics.collisionMask == null) return result;
        ...
        */
        ...
        public static void GenerateProjectIndexPage(string project, bool offline, bool menu, string scenePath, string sceneFilename, string scriptPath, string projectScript, string binaries, bool antialias, bool adaptive, string splash, string text, string ring, string spin, string datax)
        {
            ...
            string projectTemplate = Path.Combine(Application.dataPath, "Babylon/Template/Config/projects.bjs");
            if (!String.IsNullOrEmpty(projectTemplate) && File.Exists(projectTemplate))
            {
                toolText = FileTools.ReadAllText(projectTemplate);
            }
            ...
            {
                string httpModuleDll = Path.Combine(Application.dataPath, "Babylon/Template/Module/ToolkitModule.dll");
                ...
                string httpModuleTxt = Path.Combine(Application.dataPath, "Babylon/Template/Module/ToolkitModule.txt");
            ...
            // Build project memory files
            string[] mems = Directory.GetFiles(previewLibrary, "*.mem");
            if (mems != null && mems.Length > 0)
            {
                foreach (string mem in mems)
                {
                    string memFile = Path.GetFileName(mem);
                    if (!memFile.StartsWith("."))
                    {
                        string memory = Path.Combine(scripts, memFile);
                        try
                        {
                            try
                            {
                                File.Copy(mem, memory, true);
                            }
                            catch (Exception fx)
                            {
            ...
            // Build project bytecode files
            string[] bcs = Directory.GetFiles(previewLibrary, "*.bc");
            if (bcs != null && bcs.Length > 0)
            {
                foreach (string bc in bcs)
                {
                    string bcFile = Path.GetFileName(bc);
                    if (!bcFile.StartsWith("."))
                    {
                        string bytecode = Path.Combine(scripts, bcFile);
                        try
                        {
                            try
                            {
                                File.Copy(bc, bytecode, true);
                            }
                            catch (Exception fx)
                            {
                                UnityEngine.Debug.LogException(fx);
            ...
            // Build project webassembly files
            string[] wasms = Directory.GetFiles(previewLibrary, "*.wasm");
            if (wasms != null && wasms.Length > 0)
            {
                foreach (string wasm in wasms)
                {
                    string wasmFile = Path.GetFileName(wasm);
                    if (!wasmFile.StartsWith("."))
                    {
                        string webassembly = Path.Combine(scripts, wasmFile);
                        try
                        {
                            try
                            {
                                File.Copy(wasm, webassembly, true);
                            }
                            catch (Exception fx)
                            {
                                UnityEngine.Debug.LogException(fx);
            ...
            // Format tool page tempate
            toolText = toolText.Replace("###TYPE###", "tool");
            toolText = toolText.Replace("###GAME###", String.Empty);
            toolText = toolText.Replace("###OFFLINE###", offline.ToString().ToLower());
            toolText = toolText.Replace("###PROJECT###", projectScript);
            toolText = toolText.Replace("###TITLE###", Application.productName);
            toolText = toolText.Replace("###SCENE###", sceneFilename);
            toolText = toolText.Replace("###SCRIPT###", formattedScript);
            toolText = toolText.Replace("###SPLASH###", splash);
            toolText = toolText.Replace("###TEXT###", text);
            toolText = toolText.Replace("###RING###", ring);
            toolText = toolText.Replace("###SPIN###", spin);
            toolText = toolText.Replace("###DATA###", datax);
            toolText = toolText.Replace("###PATH###", scenePath.TrimStart('/').TrimEnd('/').ToLower());
            toolText = toolText.Replace("###ANTIALIAS###", antialias.ToString().ToLower());
            toolText = toolText.Replace("###ADAPTIVE###", adaptive.ToString().ToLower());
            string toolPage = Path.Combine(project, "project.js");
            FileTools.WriteAllText(toolPage, toolText);
            ...
            menuText = menuText.Replace("###SPLASH###", splash);
            menuText = menuText.Replace("###TEXT###", text);
            menuText = menuText.Replace("###RING###", ring);
            menuText = menuText.Replace("###SPIN###", spin);
            menuText = menuText.Replace("###DATA###", datax);
            ...
            gameText = gameText.Replace("###SPLASH###", splash);
            gameText = gameText.Replace("###TEXT###", text);
            gameText = gameText.Replace("###RING###", ring);
            gameText = gameText.Replace("###SPIN###", spin);
            gameText = gameText.Replace("###DATA###", datax);
        ...
        public static void BuildProjectJavaScript(string buildFolder, string javascriptFile)
        {
            ...
            try
            {
                ...
                {
                    string firelight = "0", debugger = "false", wndText = FileTools.ReadAllText(wndTemplate);
                    if (ExporterWindow.exportationOptions.FirelightAudio == true) {
                        if (ExporterWindow.exportationOptions.FirelightAudioMode == 0) {
                            firelight = "1";
                        } else if (ExporterWindow.exportationOptions.FirelightAudioMode == 1) {
                            firelight = "2";
                        }
                    }
                    if (ExporterWindow.exportationOptions.FirelightDebugMode == true) {
                        debugger = "true";
                    }
                    wndText = wndText.Replace("###FMOD###", firelight);
                    wndText = wndText.Replace("###FMODL###", debugger);
                ...
                if (assets != null && assets.Length > 0)
                {
                    foreach (var asset in assets)
                    {
                        ...
                        if (!String.IsNullOrEmpty(path) && File.Exists(path) && !Path.GetFileName(path).StartsWith("."))
                        {
                            // Note: Ignore Babylon Toolkit Folder Scripts
                            bool isLibrary = (path.IndexOf("/Babylon/Library/", StringComparison.OrdinalIgnoreCase) >= 0);
                            bool isTemplate = (path.IndexOf("/Babylon/Template/", StringComparison.OrdinalIgnoreCase) >= 0);
                            if (isLibrary == false && isTemplate == false)
                            {
        ...
        public static void BuildProjectTerrainScript(string buildFolder, string javascriptFile)
        {
            string shaderScript = "window.terrain = true;\n\n";
            DefaultAsset[] shaderVxs = Tools.GetAssetsOfType<DefaultAsset>(".vertex.fx");
            if (shaderVxs != null && shaderVxs.Length > 0)
            {
                foreach (var shaderVx in shaderVxs)
            ...
            // ..
            // Note: Store Splatmap Shader Build For Developer Reference Only
            // ..
            string shaderScriptFile = Path.Combine(buildFolder, "terrains.js");
            FileTools.WriteAllText(shaderScriptFile, shaderScript);
        }

        public static int BuildProjectTypeScript(string nodePath, string tscPath, string buildFolder, string javascriptFile, string ecmascriptVersion, string tsconfigJson = null)
        {
            int result = -1;
            ...
            if (nodeExists && tscExists)
            {
                ...
                tsconfigJson = tsconfigJson.Replace("###ECMASCRIPT###", ecmascriptVersion);
